...
def charmap_transpose(charmap, flip=False):
    if flip:
        return list(map(list, zip(*reversed(charmap))))
    else:
        return list(map(list, zip(*charmap)))

def charmap_rotate_ccw(charmap, flip=False):
    if flip:
        return list(zip(*charmap[::-1]))
    else:
        return list(zip(*charmap))[::-1]
...
platform_charmap = []
load_total = 0
platform_charmap_list = []

# spin_cycles = 3
spin_cycles = 1000000000

tilt_dirs = 4
platform_charmap_cycle = 0

for line in file_input:
    platform_charmap.append(list(line.strip()))


print("O : " + str(sum(platform_charmap, []).count("O")))
# print_charmap(platform_charmap)
print("---")
# print_charmap(charmap_transpose(platform_charmap))
platform_transposed_width = len(platform_charmap) # square inputs
...
skip_next = True
print("--- Rolling ---")
for spin in range(spin_cycles):
    if spin % 100000 == 0:
        print("spin " + str(spin))
    for tilt_dir in range(tilt_dirs):
        platform_charmap_transposed = charmap_transpose(platform_charmap)
        ...
        for col in platform_charmap_transposed:
            ...
            for idx in range(platform_transposed_width):
                if col[idx] == "O" and current_free_idx > -1:
                    col[current_free_idx] = "O"
                    col[idx] = "."
                    current_free_idx += 1
                ...
                if col[idx] == "#":
                    current_free_idx = -1
        platform_charmap = charmap_rotate_ccw(charmap_transpose(platform_charmap_transposed), True)
        ...
        # print("--- --- --- Tilt Complete --- ---- ---")
        # print_charmap(platform_charmap)
    print("--- SpinCycle complete ---")
    if platform_charmap in platform_charmap_list:
        print ("list_len: " + str(len(platform_charmap_list)) + " match idx " + str(platform_charmap_list.index(platform_charmap)) + " cycle_len " + str(len(platform_charmap_list) - platform_charmap_list.index(platform_charmap)))
        # print_charmap(platform_charmap)
        # print("===")
        # print_charmap(platform_charmap_list[platform_charmap_list.index(platform_charmap)])
        platform_charmap_cycle_start = platform_charmap_list.index(platform_charmap)
        platform_charmap_cycle_len = len(platform_charmap_list) - platform_charmap_list.index(platform_charmap)
        break
    platform_charmap_list.append(platform_charmap)
    # print_charmap(platform_charmap)

print("--- found cycle, calculating final load ---")

# print_charmap(platform_charmap_list[2])
# print("==========")
# print_charmap(platform_charmap_list[9])
# print("==========")
# print_charmap(platform_charmap_list[10])
# print("==========")

# print(platform_charmap_list)
print("cycle size : " + str(platform_charmap_cycle_len))
last_charmap_idx = ((spin_cycles - 1 - platform_charmap_cycle_start) % platform_charmap_cycle_len) + platform_charmap_cycle_start
last_charmap = platform_charmap_list[last_charmap_idx]
print_charmap(last_charmap)
print((spin_cycles-2)%7)
# print_charmap(platform_charmap)

for charmap in platform_charmap_list:
    load_total = 0
    platform_charmap_transposed = charmap_transpose(charmap)
    # print_charmap(platform_charmap_transposed)
    for col in platform_charmap_transposed:
        loads = 0
        rock_idx = []
        ...
        for idx in range(platform_transposed_width):
            if col[idx] == "O":
                rock_idx.append(idx)
        # platform_charmap_transposed = list(zip(*reversed(platform_charmap_transposed)))
        # print("-- rock indexes :" + str(rock_idx))
        loads = list(map(lambda x: platform_transposed_width - x, rock_idx))
        # print("-- loads :" + str(loads) + " total: " + str(sum(loads)))
        load_total += sum(loads)
...
load_total = 0
platform_charmap_transposed = charmap_transpose(last_charmap)
# print_charmap(platform_charmap_transposed)
for col in platform_charmap_transposed:
    loads = 0
    rock_idx = []
